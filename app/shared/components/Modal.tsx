        ...
        <div className="flex min-h-full items-center justify-center p-4 text-center">
          <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-full max-w-4xl my-8">
            <div className="ti-modal-header flex justify-between items-center px-6 py-3 border-b">
              <h6 className="modal-title text-lg font-semibold">{title}</h6>
              <button
                type="button"
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-12 gap-4">
                  {fields.map((field, index) => (
                    <div key={index} className={`col-span-${field.colSpan || 6}`}>
                      <label className="form-label font-semibold">{field.label}</label>
                      <p className="text-gray-600">{renderValue(field.value)}</p>
                    </div>
                  ))}
                </div>
              )}
